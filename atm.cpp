...
: accountNumber(accNum), pin(pinCode), balance(initialBalance) {} bool validatePIN(const string& enteredPin) const
{	return pin == enteredPin;
}	double checkBalance() const {		return balance;
}
void deposit(double amount) { if (amount > 0) {
balance += amount;
cout << "Deposit successful. Current balance: $" << balance << endl;
} else {
cout << "Invalid deposit amount." << endl;
}
}	void withdraw(double amount) {	if (amount > 0 && amount <= balance) {		balance -= amount;		cout << "Withdrawal successful. Current balance: $" << balance << endl;
...
Account newAccount(accNum, pinCode, initialBalance); accounts[accNum] = newAccount;
}
...
Account* account = atm.getAccount(accountNumber); if (account) {
cout << "Enter PIN: "; cin >> pin;
if (account->validatePIN(pin)) { int choice;	do {
cout << "\nATM Menu:\n";
...
<< endl;	break;	case 2: {
double depositAmount;		cout << "Enter amount to deposit: ";	cin >> depositAmount;	account-
>deposit(depositAmount);	break;
}	case 3: {
double withdrawAmount;		cout << "Enter amount to withdraw: ";			cin >> withdrawAmount;	account-
>withdraw(withdrawAmount);	break;
}
case 4:
